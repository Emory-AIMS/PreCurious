...
def parse_args():
    ...
    parser.add_argument("--rewind", nargs="+", type=str, default=[])
...
def train_ft(
    ...
    for epoch in range(args.num_train_epochs):
        ...
        if (not args.only_eval_last) or epoch == args.num_train_epochs - 1:
            mia_m, ppl_m = priv_measure(
                model=model,
                accelerator=accelerator,
                args=args,
...
def main():
    ...
    folder_name = f"un_ft_{args.peft}_{args.model_name_or_path}_{args.dataset_name}_aux{args.pre_aux_ratio}_lr{args.learning_rate}_epoch_{args.num_train_epochs}_def{args.dropout_def}{args.weight_decay_def}{args.clip_def}_head{args.drop_head}{args.untied_head}_init{init_model_name}_REWIND{'_'.join(args.rewind)}"
    ...
    if args.load_init_file:
        ...
        if len(args.rewind) > 0:
            for subname in args.rewind:
                # Create a list of keys to iterate over
                keys_to_remove = [n for n in ckpt['model_state_dict'].keys() if subname in n]
                for n in keys_to_remove:
                    # Remove the key from the dictionary
                    ckpt['model_state_dict'].pop(n, None)
                    logger.info(f'Rewinding {subname} with {n}')
        if args.drop_head:
