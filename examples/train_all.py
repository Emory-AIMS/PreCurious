...
def parse_args():
    ...
    parser.add_argument("--num_train_epochs_break", type=int, default=None, help="Total number of training epochs to perform.")
    parser.add_argument(
    ...
    parser.add_argument("--bad_key", type=str, default='attn.c_proj.weight', help="add additional bad tricks")
    ...
    parser.add_argument("--load_ref_file", default=None, type=str)
    parser.add_argument("--load_ref_name", default=None, type=str)
    ...
    parser.add_argument('--split_dir', type=str, default='../cache/datasets/', help='folder for saving data split')
    ...
    parser.add_argument("--save_ft", action="store_true", help="If true, turn on wandb logging")
    ...
    parser.add_argument('--balance', action='store_true', help='balanced query set on the number of mem and non mem samples')
    parser.add_argument('--non_dir', type=str, default=None)
    parser.add_argument('--train_aux_dir', type=str, default=None)
...
def train_pre_adv(
    ...
    logger.info("***** Running Pre-Curious / reference training *****")
    ...
    for epoch in range(args.pre_num_epochs):
        ...
        if epoch + 1 in args.save_ckpt:
            ...
            model_path = args.model_folder + f'/pre_ckpt_{epoch+1}.pth'
...
def train_ft(
    ...
    # Prepare everything with our `accelerator`.
    model, model_ref, optimizer, train_dataloader, train_dataloader_mia, eval_dataloader, non_dataloader = accelerator.prepare(
        model, model_ref, optimizer, train_dataloader, train_dataloader_mia, eval_dataloader, non_dataloader
    )
    ...
    mia_m, ppl_m = priv_measure(
            ...
            train_dataloader=train_dataloader_mia,
    ...
    for epoch in range(args.num_train_epochs):
        if args.num_train_epochs_break is not None and epoch == args.num_train_epochs_break - 1:
            break
        model.train()
        running_loss = deque()
...
def main():
    ...
    peft_name = args.peft_pre + args.peft

    att_name = args.attack
    
    folder_name = f"all_{args.attack}_{args.model_name_or_path}_{args.dataset_name}_aux{args.pre_aux_ratio}_{peft_name}_{att_name}_pe{args.pre_num_epochs}_f{args.mode}_p{args.p}_plr{args.learning_rate_pre}"
    ...
    if args.train_aux_dir is not None:
        assert args.non_dir is not None
        train_dataset_mia = load_from_disk(args.train_aux_dir)
        non_dataset = load_from_disk(args.non_dir)
        train_dataloader_mia = DataLoader(
            train_dataset_mia, shuffle=True, collate_fn=default_data_collator, batch_size=args.per_device_train_batch_size
        )
    else: # default
        train_dataloader_mia = DataLoader(
    ...
    if args.balance and len(non_dataset) != len(train_dataloader_mia.dataset):
        assert len(non_dataset) > len(train_dataloader_mia.dataset)
        idx_list = list(range(len(train_dataloader_mia.dataset)))
        non_dataset = Subset(non_dataset, idx_list[:len(train_dataloader_mia.dataset)])
    ...
    if args.tune_ref and args.attack is None and args.only_eval_last:
        args.attack = 'warm'
        train_pre_adv(
            model_ref,
            args,
        ...
        model, model_ref, eval_dataloader, train_dataloader, train_dataloader_mia, non_dataloader = accelerator.prepare(
            model, model_ref, eval_dataloader, train_dataloader, train_dataloader_mia, non_dataloader
        )
        mia_m, ppl_m = priv_measure(
            model=model,
            accelerator=accelerator,
