...
from private_transformers.autograd_grad_sample import requires_grad
...
def parse_args():
    ...
    parser.add_argument(
        ...
        type=str,
        default=None,
        choices=['full', 'head', 'layers', 'bitfit', 'adapter', 'prefix', 'compacter', 'lora', 'ia3', 'loradp']
    )
    parser.add_argument(
        "--adapter_reduction",
        type=int,
    ...
    parser.add_argument("--dp_eps", default=None, type=float, help='privacy budget for ft training')
    parser.add_argument("--dp_norm", default=1, type=float, help='privacy budget for ft training')
    ...
    parser.add_argument('--prefix_mode', type=str, default='train_dup', choices=['train_dup', 'other_mode'], help='Mode of the prefix.')
    ...
    assert args.dp_eps is not None
    assert args.clip_def is None
...
def train_ft(
    ...
    if args.dp_eps is not None:
        privacy_engine = PrivacyEngine( # TODO:add params
            model,
            batch_size=total_batch_size,
            sample_size=len(train_dataloader.dataset),
            epochs=args.num_train_epochs,
            max_grad_norm=args.dp_norm,
            target_epsilon=args.dp_eps,
            target_delta=math.pow(len(train_dataloader.dataset), -1.1),
        )
    ...
    model, model_ref, train_dataloader, eval_dataloader, non_dataloader = accelerator.prepare(
        model, model_ref, train_dataloader, eval_dataloader, non_dataloader
    )
    ...
    for epoch in range(args.num_train_epochs):
        ...
        model.train()
        running_loss = deque()
        running_r_clip = deque()
        running_grad_norm = deque()
        logger.info(f"training epoch {epoch}")
        for step, batch in enumerate(train_dataloader):
            labels = batch["labels"][..., 1:].contiguous()
            logits = model(**batch).logits[..., :-1, :].contiguous()
            
            losses = loss_fn(logits.transpose(1, 2), labels).mean(1)
            loss = losses.mean()
            running_loss.append(loss.item())
            loss = loss / args.gradient_accumulation_steps
            if (step + 1) % args.gradient_accumulation_steps == 0 or step == len(train_dataloader) - 1:
                optimizer.step(loss=losses)
                lr_scheduler.step()
                running_r_clip.append(np.mean(privacy_engine.r_clip))
                running_grad_norm.append(np.mean(privacy_engine.norm_samples))
                optimizer.zero_grad()
                # progress_bar.update(1)
                completed_steps += 1
            else:
                optimizer.virtual_step(loss=losses)
        ...
        if ((epoch + 1) % args.log_per_step == 0 and not args.only_eval_last) or epoch == args.num_train_epochs - 1:
            ...
            if epoch == args.num_train_epochs - 1 and args.save_ft:
                ...
                elif args.peft in ['head', 'bitfit', 'full', 'loradp']:
...
def main():
    ...
    folder_name = f"un_ftdp_{args.peft}_{args.model_name_or_path}_{args.dataset_name}_aux{args.pre_aux_ratio}_lr{args.learning_rate}_epoch_{args.num_train_epochs}_def{args.dropout_def}{args.weight_decay_def}{args.clip_def}_head{args.drop_head}{args.fresh_head}_init{init_model_name}_{args.bad}_eps{args.dp_eps}"
    ...
    if args.load_init_file is not None:
        if 'adapter' in args.load_init_file:
        ...
        elif 'lora' in args.load_init_file:
    ...
    # TODO: add drop head and other components
    count_parameters(args, logger, model, prefix='ft')
